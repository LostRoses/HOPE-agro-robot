...
## Descrição Geral
O código controla o robô agrícola em dois modos:
- **Modo Manual**: movimentação via controle remoto IR.  
- **Modo Autônomo**: movimentação baseada em sensor ultrassônico HC-SR04, com lógica de desvio de obstáculos.

## Principais Componentes
- **Bibliotecas**: Servo.h, IR_remote.h, Keymap.h.  
- **Sensores**: HC-SR04 para medir distância.  
- **Atuadores**: Motores DC controlados pelo driver L298N e servo motor.  
- **Controle**: Alternância entre manual e autônomo via controle IR.
...
void decidirAcaoPorDistancia(float d) {
  ...
  if (d <= 10) {
    Serial.println(" Obstáculo muito próximo. Executando desvio alternativo...");
